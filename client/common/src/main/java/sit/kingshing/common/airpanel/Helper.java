...
final class Helper implements Contract.Helper {
    ...
    private static int virtualBarHeight; //虚拟键盘高度
    private static int notchHeight;  //刘海高度
    ...
    public void setup(Activity activity) {
        if (mView.getId() == R.id.airPanelSubLayout) {
            mTarget = new Helper.Sub(mView, mAttribute);
        } else {
            ...
            mTarget = new Helper.Boss((Contract.Panel) childPanel);
        }
    ...
    private static class Boss implements Contract.Helper {
        ...
        public void setup(Activity activity) {
            ...
            */
            ...
            //获取屏幕
            /**
             * 使用新的方法，无论手机的虚拟导航栏或标题栏是否显示隐藏都可以正确获取分辨率
             */
            Context context = activity.getApplicationContext();
            DisplayMetrics dm = new DisplayMetrics();
            WindowManager windowMgr = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
            final Display display = windowMgr.getDefaultDisplay();
            ...
            Log.e("getMetrics", "" + dm.heightPixels);
            virtualBarHeight = Util.getVirtualBarHeight(activity);
             notchHeight = Util.getNotchParams(activity);
        }
